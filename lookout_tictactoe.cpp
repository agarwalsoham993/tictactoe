...
void play_normal(int (*values)[9],int n){// if n = 1 user starts ; n==0 bot starts
    print_grid(values);
    ...
    play_normal(values,(n+1)%2);
}

int final_d(int (*values)[9],int n){
    int t = check_grid(values);
    if(t!=2){
        return t;
    }
...
void optimal_move(int (*values)[9],int n){
    int t = to_win(values,n);
    if(t!=100){    // if can win ,then winning is firstly imp 
        (*values)[t] = n;
        return;
    ...
    int optimal[count];int opt_count=0;
    for(int i=0;i<count;i++){
        copy(&((*values)[0]),&((*values)[0])+9,values_copy);
        values_copy[best[i]]=n;
        if (final_d(&values_copy,(n+1)%2)!=(n+1)%2){
            optimal[opt_count++]=best[i];
        }
    }
    if (opt_count == 0){
        int ind = best[randint(0, count)];
    ...
    int ind = optimal[randint(0, opt_count)];
    (*values)[ind] = n;
    return;
}

void play_optimal(int (*values)[9],int n){// if n = 1 user starts ; n==0 bot starts
    print_grid(values);
    ...
    if (n==0){
        ...
        optimal_move(values,0);
    }
    play_optimal(values,(n+1)%2);
}

int optimal_v_normal(int (*values)[9],int n){ // n=0 means start with optimal otherwise normal
    if(n==0){
        int t = check_grid(values);
        if(t!=2){
            return t;
        }
        optimal_move(values,n);
        n=1;
    }
    if(n==1){
        int t = check_grid(values);
        ...
        normal_move(values,n);
        n=0;
    }
    return optimal_v_normal(values,n);
}

int main(){
    srand((unsigned)time(0)); 
    // int values[9] = {2,2,2,2,2,2,2,2,2};
    // play_optimal(&values,1);

    int win_x = 0;int lose_x =0;int draw = 0;int t;
    for(int i=0;i<1000;i++){
        int values[9] = {2,2,2,2,2,2,2,2,2};
        t = optimal_v_normal(&values,i%2);
        if(t==0) win_x++;
        else if (t==1) lose_x++;
        else if (t==3) draw++;
    }

    cout << "\n no of wins for bot " << win_x << "\n no of loss for bot" << lose_x << "\n no of draws" << draw << "\n";
